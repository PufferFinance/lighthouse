//! This build script downloads the latest SecureSigner release and places it in the `OUT_DIR` so it
...
pub async fn download_binary(dest_dir: PathBuf) {
    ...
    let client = Client::builder()
        ...
        .user_agent("securesigner_tests")
        .build()
        .unwrap();

    let version = if let Some(version) = FIXED_VERSION_STRING {
        version.to_string()
    } else if let Ok(env_version) = env::var("LIGHTHOUSE_SECURESIGNER_VERSION") {
        env_version
    } else {
    ...
    // Rename the securesigner directory so it doesn't include the version string. This ensures the
    // path to the binary is predictable.
    let securesigner_dir = dest_dir.join("securesigner");
    if securesigner_dir.exists() {
        fs::remove_dir_all(&securesigner_dir).unwrap();
    }
    fs::rename(
        dest_dir.join(format!("securesigner-{}", version)),
        securesigner_dir,
    )
    .unwrap();
