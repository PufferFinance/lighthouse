...
//! SecureSigner by Puffer Finance.
//!
//! These tests aim to ensure that:
//!
//! - Lighthouse can issue valid requests to SecureSigner.
//! - The signatures generated by SecureSigner are identical to those which Lighthouse generates.
//!
//! There is a build script in this crate which obtains the latest version of SecureSigner and makes
...
mod tests {
    ...
    /// If the we are unable to reach the SecureSigner HTTP API within this time out then we will
    /// assume it failed to start.
    const UPCHECK_TIMEOUT: Duration = Duration::from_secs(20);

    /// Set to `false` to send the SecureSigner logs to the console during tests. Logs are useful when
    /// debugging.
    const SUPPRESS_SECURESIGNER_LOGS: bool = false;

    type E = MainnetEthSpec;

    /// This marker trait is implemented for objects that we wish to compare to ensure SecureSigner
    ...
    /// A file format used by SecureSigner to discover and unlock keystores.
    #[derive(Serialize)]
    struct SecureSignerKeyConfig {
        #[serde(rename = "type")]
        config_type: String,
    ...
    const SECURESIGNER_LISTEN_ADDRESS: &str = "127.0.0.1";
    ...
    /// The location of the SecureSigner binary generated by the build script.
    fn securesigner_binary() -> PathBuf {
    ...
    fn root_certificate_path() -> PathBuf {
        tls_dir().join("lighthouse").join("securesigner.pem")
    }
    ...
    /// A testing rig which holds a live SecureSigner process.
    struct SecureSignerRig {
        keypair: Keypair,
        _keystore_dir: TempDir,
        keystore_path: PathBuf,
        // securesigner_child: Child,
        http_client: Client,
        url: Url,
    }

    impl Drop for SecureSignerRig {
        fn drop(&mut self) {
            // self.securesigner_child.kill().unwrap();
        }
    }

    impl SecureSignerRig {
        pub async fn new(network: &str, listen_address: &str, listen_port: u16) -> Self {
            ...
            let key_config = SecureSignerKeyConfig {
            ...
            let tls_keystore_file = tls_dir().join("securesigner").join("key.p12");
            let tls_keystore_password_file = tls_dir().join("securesigner").join("password.txt");
            let tls_known_clients_file = tls_dir().join("securesigner").join("known_clients.txt");

            let stdio = || {
                if SUPPRESS_SECURESIGNER_LOGS {
                    Stdio::null()
                } else {
                    Stdio::inherit()
                }
            ...
            // let securesigner_child = Command::new(securesigner_binary())
            //     .arg(format!(
            //         "--key-store-path={}",
            ...
            let url = Url::parse(&format!("http://localhost:4242")).unwrap();
            println!("url: {:?}", url);
    ...
    /// A testing rig which holds multiple `ValidatorStore` rigs and one `SecureSigner` rig.
    ///
    /// The intent of this rig is to allow testing a `ValidatorStore` using `SecureSigner` against
    /// another `ValidatorStore` using a local keystore and ensure that both `ValidatorStore`s
    /// behave identically.
    struct TestingRig {
        _signer_rig: SecureSignerRig,
    ...
    impl TestingRig {
        pub async fn new(network: &str, spec: ChainSpec, listen_port: u16) -> Self {
            let signer_rig =
            SecureSignerRig::new(network, SECURESIGNER_LISTEN_ADDRESS, listen_port).await;
            let validator_pubkey = signer_rig.keypair.pk.clone();
            println!("validator_pubkey: {:?}", validator_pubkey);
            ...
            println!("after local_signer_validator_store 376");
            ...
            let remote_signer_validator_store = {
                let validator_definition = ValidatorDefinition {
                    ...
                    builder_proposals: None,
                    description: String::default(),
                    signing_definition: SigningDefinition::Web3Signer(Web3SignerDefinition { // TODO
            ...
            println!("after remote_signer_validator_store 397");
    ...
    async fn test_base_types(network: &str, listen_port: u16) {
        ...
        TestingRig::new(network, spec.clone(), listen_port)
            ...
            .assert_signatures_match("attestation", |pubkey, validator_store| async move {
                ...
                // Secure-Signer must have target > 0
                attestation.data.source.epoch += 1;
                attestation.data.target.epoch += 2;
                let target = attestation.data.target.epoch.clone();
                println!("in attestation 491, {:?}", attestation);
                validator_store
                    .sign_attestation(pubkey, 0, &mut attestation, target)
                    .await
                    .unwrap();
                attestation
            })
    ...
    async fn mainnet_altair_types() {
        test_altair_types("mainnet", 4242).await
    }

    #[tokio::test]
    async fn prater_base_types() {
        test_base_types("prater", 4242).await
    }

    #[tokio::test]
    async fn prater_altair_types() {
        test_altair_types("prater", 4242).await
    }

    #[tokio::test]
    async fn ropsten_base_types() {
        test_base_types("ropsten", 4242).await
    }

    #[tokio::test]
    async fn ropsten_altair_types() {
        test_altair_types("ropsten", 4242).await
    }

    #[tokio::test]
    async fn ropsten_merge_types() {
        test_merge_types("ropsten", 4242).await
    }


    #[tokio::test]
    async fn securesigner_integration() {
        test_base_types("mainnet", 4242).await;
        test_altair_types("ropsten", 4242).await;
        test_merge_types("ropsten", 4242).await
    }
